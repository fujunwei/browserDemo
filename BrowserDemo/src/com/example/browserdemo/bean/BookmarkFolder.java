...
public class BookmarkFolder implements Serializable {
	...
	private List<Bookmark> containedBookmarks = new ArrayList<Bookmark>();
	private List<BookmarkFolder> containedFolders = new ArrayList<BookmarkFolder>();
	...
	/**
	 * Method will look for a bookmark object with the given URL and searches in
	 * this folder's sub-folders if necessary
	 * 
	 * @param url
	 *            - URL to search for
	...
	/**
	 * Method search all folders and returns an ArrayList of bookmarks and
	 * searches in this folder's sub-folders if necessary
	 */
	 ...
	 * @return
	 */
	public boolean moveFolderToAnotherFolder(BookmarkFolder moveFolder,
			BookmarkFolder anotherFolder) {
		BookmarkFolder folder = getFolderDeep(anotherFolder.getInternalName());
		if (folder != null) {
			folder.addFolder(moveFolder);
			return true;
		}
		return false;
	}

	public boolean moveBookMarkToAnotherFolder(Bookmark bookMark,
			BookmarkFolder anotherFolder) {
		BookmarkFolder folder = this.getFolderDeep(anotherFolder
				.getInternalName());
		if (folder != null) {
